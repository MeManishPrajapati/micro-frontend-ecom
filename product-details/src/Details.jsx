...
export default function Details({ productId: initialId }) {
    ...
    const [productId, setProductId] = useState(initialId || getIdFromQuery() || "1");
    ...
    useEffect(() => {
        ...
        async function fetchProduct() {
            ...
            try {
                ...
                const res = await fetch(`https://fakestoreapi.com/products/${encodeURIComponent(productId)}`, { signal });
                if (!res.ok) {
    ...
    return (
        <div style={{ maxWidth: 800, margin: "16px auto", fontFamily: "system-ui, Arial" }}>
            <div style={{ marginBottom: 12 }}>
                <label style={{ marginRight: 8 }}>Product ID:</label>
                <input value={productId} onChange={onChangeId} style={{ width: 80, marginRight: 8 }} />
            ...
            {!loading && !error && product && (
                <div style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
                    <img
                        src={product.image}
                        alt={product.title}
                        style={{ width: 200, height: 200, objectFit: "contain", border: "1px solid #eee", padding: 8 }}
                    />
                    <div>
            ...
            {!loading && !error && !product && (
                <div>No product loaded. Enter an ID (1-20 for fakestoreapi) and click Load.</div>
            )}
        </div>
    );
}